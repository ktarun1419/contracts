...
contract saleContract is Ownable{ 
  ...
  event SwapAndLiquify(
        ...
        if(totalBnbReceived > softCap){
         ...
         //   lockingVault lockLiquidity = lockingVault(0xA99aA252E42C41d16249801b30748BaBa0F2EbB2);
            ...
            )
            {
                ...
                uniswapV2Pair = uniswapV2Factory.getPair(address(token), address(paymentToken));
    ...
    function participateAndPay(uint256 amount) public payable {
    if (whitelist == true){
        require(isWhitelisted[msg.sender] == true, "Not Whitelisted ");
    }
    IERC20 sendingToken=IERC20(paymentToken);
    require(sendingToken.allowance(msg.sender, address(this))>=amount,"Please approve the token");
    sendingToken.transferFrom(msg.sender, address(this), amount);
    ...
    require(totalBnbReceived.add(amount) <= maxCap, " purchase would exceed max cap");
    require(buyByUser[msg.sender].add(amount) <= maxAllocaPerUser ,"limit reached");
    require(buyByUser[msg.sender].add(amount) >= minAllocaPerUser ,"investing limit reached");
    buyByUser[msg.sender] = buyByUser[msg.sender].add(amount);
    totalBnbReceived = totalBnbReceived.add(amount);
  }
}
contract HAMILaunchPad is Ownable, constructorLibrary {
  ...
  bool public whitelist;
  mapping (address => bool) public isWhitelisted;
  address deploy=0xd9145CCE52D386f254917e481eB44e9943F39138;
  event SwapAndLiquify(
        uint256 tokensSwapped,
        uint256 ethReceived,
        uint256 tokensIntoLiquidity
  ...
  
  {
     ...
     */
    function participateandPay(uint256 amount) public {
         (bool success, bytes memory data) = deploy.delegatecall(
            abi.encodeWithSignature("participateAndPay(uint256)",amount)
        );
    }
...
//   }
  function sendValue(address payable recipient, uint256 amount) internal {
    ...
    function finalizeSale(address _deploy) public  onlyOwner{
        (bool success, bytes memory data) = _deploy.delegatecall(
            abi.encodeWithSignature("finalizeSale()")
        );
    }
    function claimTokens(address _deploy) public onlyOwner{
        (bool success, bytes memory data) = _deploy.delegatecall(
            abi.encodeWithSignature("claimTokens()")
        );
    }
    function cancelSale(address _deploy) public onlyOwner{
         (bool success, bytes memory data) = _deploy.delegatecall(
            abi.encodeWithSignature("cancelSale()")
        );
    }
...
contract HAMILaunchPadDeployer is Ownable, constructorLibrary{
    ...
    function deployProjectOnLaunchpad(parameter memory params) public payable  returns(address) {
       IERC20 payableToken=IERC20(params.tokenAddress);
        require(payableToken.allowance(msg.sender, address(this))>params.tokenAmount);
        bool success=payableToken.transferFrom(msg.sender, address(this), params.tokenAmount);
        require(success ,"token sending failed");
        uint256 maxCap = (params._numberOfIdoTokensToSell) * (params._tokenPriceInBNB);
        uint256 participationBalanceTokens = (maxCap).mul( 10 ** (params.tokenDecimals) ).div(params._tokenPriceInBNB);
        uint256 liquidityBalanceTokens = participationBalanceTokens.mul(params.liquidityPercentage).div(100);
        uint256 launchPadBalanceTokens = participationBalanceTokens.mul(launchPadServiceFeePercentage).div(100);
